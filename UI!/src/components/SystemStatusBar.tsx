...
export function SystemStatusBar({ fleet }: { fleet: FleetState }) {
  ...
  // --- NEW: Calculate dynamically active drones ---
  const activeDronesCount = drones.filter(d => d.status !== "CHARGING" && !d.abortingToCenter).length;
  // Note: I also filter out 'abortingToCenter' so if they are flying home dying, they aren't counted as actively searching.
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 5, background: "rgba(51,250,255,0.07)", border: `1px solid ${AI_CYAN}33`, borderRadius: 16, padding: "3px 11px", flexShrink: 0 }}>
        ...
        <span style={{ color: LABEL_PRIMARY, fontSize: 14, fontFamily: "'Roboto Mono',monospace", fontWeight: 700, textShadow: LABEL_GLOW }}>AUTONOMOUS · {activeDronesCount} DRONES</span>
      </div>
      ...
      {/* --- NEW: Dynamic Fleet Ratio --- */}
      <div style={{ display: "flex", gap: 20, fontFamily: "'Roboto Mono',monospace" }}>
        {[
          { label: "FLEET", value: `${activeDronesCount}/${MAX_FLEET}`, color: atCap ? CRIT_RED : LABEL_PRIMARY },
